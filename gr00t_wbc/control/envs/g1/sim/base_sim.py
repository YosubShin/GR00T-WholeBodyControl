...
from gr00t_wbc.control.envs.g1.sim.metric_utils import check_contact, check_height
from gr00t_wbc.control.envs.g1.sim.sim_utilts import get_subtree_body_names
from gr00t_wbc.control.envs.g1.sim.unitree_sdk2py_bridge import ElasticBand, UnitreeSdk2Bridge
...
class DefaultEnv:
    ...
    def compute_hand_qpos(self) -> np.ndarray:
        ...
        if self.unitree_bridge is not None and self.unitree_bridge.low_cmd:
            for i in range(self.unitree_bridge.num_hand_motor):
                scene_i = self.unitree_bridge.get_scene_hand_joint_index(i)
                hand_qpos[scene_i] = self.unitree_bridge.get_hand_motor_cmd("left", i).q
                hand_qpos[scene_i + self.num_hand_dof] = self.unitree_bridge.get_hand_motor_cmd(
                    "right", i
                ).q
        return hand_qpos
